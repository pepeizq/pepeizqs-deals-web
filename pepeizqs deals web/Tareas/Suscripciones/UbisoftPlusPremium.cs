...
{
	public class UbisoftPlusPremium : BackgroundService
	{
		private string id = "ubisoftpluspremium";

		private readonly ILogger<UbisoftPlusPremium> _logger;
		private readonly IServiceScopeFactory _factoria;
		private readonly IDecompiladores _decompilador;

		public UbisoftPlusPremium(ILogger<UbisoftPlusPremium> logger, IServiceScopeFactory factory, IDecompiladores decompilador)
		{
			_logger = logger;
			_factoria = factory;
				...
				if (piscinaTiendas == piscinaUsada)
				{
					...
					if (conexion.State == System.Data.ConnectionState.Open)
					{
						...
						if (sePuedeUsar == true && Admin.ComprobarTiendasUso(conexion, TimeSpan.FromSeconds(60)) == null)
						{
							try
							{
								await APIs.Ubisoft.Suscripcion.BuscarPremium(conexion);
